...
> 当前状态（2026-07-29）：这是历史实施记录。实验性第 13—15 关及其地图、
> 配置和发布入口已经撤下；稳定 MOD 只发布 12 个原版关卡，项目后续主线为
> `Remake/` 的完整复刻。下文旧数量与扩展关数据仅作历史证据。
...
- m014 先修复了跨图路径与渲染/命中相机不同步；随后“真实移动”专项
  ...
  单逻辑核占用 87.5%。最终采用引擎语义安全地表合成，196/24,000 格在
  相同 `TileGroupMapId` 内发生视觉变化，资源组、碰撞、事件、scene 和
  巡逻拓扑不变。双点击正式回归的两个目标均精确到达，反向帧为 0，
  P95/P99 8.113/12.630ms，0 次未响应和光标裁剪。详见
