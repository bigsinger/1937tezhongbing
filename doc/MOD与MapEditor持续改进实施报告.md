...
- `SDK/mission-routes.json` 是 1—15 关的唯一关卡路由源，代理、启动中心
  和回归探针共同使用。
- 15 关现已在原版“新游戏”之后显示游戏进程内的 JSON 驱动文字标题、
  剧情和三项目标，确认后通过原版自己的简报确认标志跳过旧图片，不模拟
  鼠标；`Mod/关卡名称.json` 的修改无需重编译即可在下次任务生效。
- GFL 索引 1048—1059 保留空记录以维持全部固定编号，旧简报图片载荷共
  清除 5,461,822 字节；第 1、12 关使用精简归档完成原引擎进图验证。
...
- m014 后续专项复测发现两条跨全图巡逻导致原版单线程路径重算，单逻辑核
  占用约 42.5%；改为等容量局部短循环并加入 64 步生成门禁后降至 9.2%，
  0 次未响应，完整相机跨度 3044×2589。详见
  [扩展关卡与文字简报修复报告](扩展关卡与文字简报修复报告-20260727.md)。
