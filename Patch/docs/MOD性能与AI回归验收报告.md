...
> 当前状态（2026-07-29）：本文是历史性能快照。实验性第 13—15 关已经
> 撤下，当前稳定 MOD 与自动回归范围均为 12 个原版关卡。
...
每关分别覆盖任务启动、F1、M、鼠标、AI 警报、保存、读取、失败、
失败后重玩和胜利，共 150 个阶段。原版 12 关和扩展关“余烬行动”、
“锄奸行动”“破晓密令”全部通过：

- 15/15 关通过，150/150 阶段通过；
- 未响应次数 0，光标裁剪限制样本 0；
- 各关合成器 P99 为 8.633—13.047 ms；
- 各关 CPU 为 4.89%—12.51%；
...
性能、光标、15 关闭环和 AI 约束均达到发布门槛。可重复执行：
