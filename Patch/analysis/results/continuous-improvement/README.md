...
- `mod-regression-summary.md/json`：15 关十阶段回归；
...
- `cursor-operability-summary.md/json`：菜单/场景只读裁剪监测与进程内
  客户区回放，确认系统光标未被限制；
- `map-editor-validation.md`：15 张真实 VWF、1,037 素材和功能矩阵。
...
最终 15 关共通过 150 个阶段，未响应与光标裁剪限制均为 0；AI 搜索
44 次、重规划 176 次、脱离成功 44/44，最大 tick 为 620 μs，且警报后
没有采样视线外玩家的实时位置。
