...
- 鼠标进入战场四边的 32 像素渐变区会平滑加速卷屏；下边缘以底部工具栏上沿为准，因此窗口和全屏模式都可垂直卷屏。左右/下方向键、中键拖动和滚轮缩放仍可辅助操作（原版 `↑` 是强制目标按住键）；光标使用原 GFL 16 `mouse.spr`，且不会捕获、吸附或移动系统鼠标；
...
- 167 个带移动脚印的交互门在规划阶段即作为 A* 通道；关门仍保持原图和 L2 视线遮挡，角色靠近时自动开门，玩家也可直接点击。这样长路径不会在门前丢失目标。
- 敌人移动动画按 SPR 步幅、保持 tick 和当前速度换算帧间隔，不再用固定慢帧率造成“滑行”。产品模式会把相邻巡逻兵组织成一名路线领队和最多三名跟随者；跟随者使用旋转队形槽、独立 A*、追赶限速，并随领队停步和转向。
- 底栏本身拦截世界输入；五名角色、当前武器、观察、地图和系统按钮都有提示。当前武器区紧邻右侧动作区，点击与 `Tab` 相同，会切换下一件武器。
- 活物选择和战斗状态只通过头顶血条表达，不再绘制红色地面圆圈。缺少原图的弹药与物品改用弹药箱/物资包占位图，不再显示容易误解的“圆圈减号”。
