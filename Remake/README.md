...
|---|---|
| [![真实关卡、原版 HUD 与现代运行时](../Screenshots/Remake-gameplay-hud.jpg)](../Screenshots/Remake-gameplay-hud.jpg) | [![现代设置、显示与辅助功能](../Screenshots/Remake-modern-settings.jpg)](../Screenshots/Remake-modern-settings.jpg) |
| **十二关历史档案与教育内容** | **性能、导航、感知与 AI 诊断** |
| [![十二关历史档案与教育内容](../Screenshots/Remake-history-archive.jpg)](../Screenshots/Remake-history-archive.jpg) | [![性能、导航、感知与 AI 诊断](../Screenshots/Remake-developer-diagnostics.jpg)](../Screenshots/Remake-developer-diagnostics.jpg) |
...
本轮已在不依赖 `Patch`/`Mod` 运行时代码的前提下完成渐进式现代化：性能与
首次操作门禁、服务拆分、空间索引、感知错峰、后台关卡读取、经典/现代双操作、
规则集与难度分离、schema 4 存档、辅助功能、中英文产品文本、AI 反馈、十二关
历史档案、诊断包和自包含发行链。范围、证据与未覆盖真机边界见
[现代化改造实施与验收计划](docs/现代化改造实施与验收计划.md)、
[现代化改造验收报告](docs/现代化改造验收报告.md)和
[现代化版本发布说明](docs/现代化版本发布说明.md)。试玩反馈形成的追击跑动、
卡位恢复、拾取优先、空地强制开火、警戒归队、动态视线与 HUD 安全区等通用修复，
见[现代化体验稳定化实施与验收](docs/现代化体验稳定化实施与验收.md)。

现代化第二轮进一步把会影响胜负的系统统一到 60 Hz 整数模拟 tick，并完成事件
空间索引、预防式移动预约、响应式中英文 UI、检查点快速重试、战术暂停与顺序命令
队列、有限证据 AI 黑板、产品级命令回放，以及安全的 `.m1937pack` 原生内容生态。
MapEditor 可以用相同 schema 导出并在独立临时目录一键试玩合成关卡；内容包不
允许携带脚本或可执行文件。实施范围、具体文件和最终门禁数据见
[现代化第二轮持续改进实施方案](docs/现代化第二轮持续改进实施方案.md)、
[现代化第二轮实施日志](docs/现代化第二轮实施日志.md)和
[现代化第二轮验收报告](docs/现代化第二轮验收报告.md)，内容制作见
[原生内容包与 MapEditor 工作流](docs/原生内容包与MapEditor工作流.md)。

## 里程碑状态

| 层次 | 当前成果 | 尚未完成 |
|---|---|---|
| 资源格式 | 1394 个 GFL 条目；IBLOCK、TLG1、SPR1、DBL、VWF、SLIST1、SLF/WAV；L1—L5、阵营、感知、生命值、攻击和巡逻字段；41 个 version-5 actor 槽均已定性，两个无消费者全零槽及 24 个加载器丢弃尾值由 19,199 实体机器目录持续审计 | 其他发行版/编辑器私有 L4/L5 写入流程，不阻塞当前稳定 MOD 十二关运行 |
...
| 关卡 | `m000`—`m011` 全部生成地形、19,199 个实体和 L2/L3 数据；十二关原始 Layer 3 的 117,112 个静态格（含 6,710 个编码非角色 scene 足印）逐格保持 solid，768 个活动角色的 1,233 个源格才转交动态占位；十类拾取物已按 DBL `header[2]`、原容器/mode、单件数量、精灵点击框和 32×16 邻格完成规则恢复，汽油桶保持场景爆炸物 | 通用中立角色行为与外部真人长时间验收 |
...
| 玩法 | 原版反向寻路、十二关玩家障碍路线的实际轨迹形状差分（4 像素硬门限、最差 3.420 像素）、事件空间索引与动态占位、确定性格预约和窄通道会车、稀疏图实时动态规划与密集图巡逻预热/后缀/静态失败复用、多格足印不可达精确预检、门增量导航、实际战斗；60 Hz 整数模拟 tick 与产品命令回放；四渲染队列；右侧五列背包；6/7/9 投射物；type 8/10 的 actor 62 精确伤害/几何/警报/五轮效果粒子，actor 61/62 主体、type 63—65 永久焦痕及 type 72—76 抛物线弹跳碎片，与 type 11 注意力保持；S 的方向/L2 遮挡空心扇形、type 90/GFL 341 唯一观察标记与 B 的 type 78/GFL 64 藏尸/双容器/101 tick 闭环；实时右下角地图；启动页/菜单内十二关原生自由选择；十槽原子存档、schema 0—3→4 迁移、滚动检查点和快速重试、运行时/内容/工作区身份与未来版本只读保护、原 SAV/SI 单向导入及真实关卡恢复、十二关胜利进度磁盘闭环、按键重映射、分通道音量、失败重玩；现代规则战术暂停/顺序命令队列、证据黑板与协同搜索；声明式 `.m1937pack` 和 MapEditor 导出/试玩；原 WAV/简报、原版唯一两段启动影片顺序、确定性事件回放和十二关双轮十分钟窗口性能门禁；十二关原版启动随机流检查点、772 名角色构造值、656 名观察门角色顺序、初始化后 769 条首轮完整调用及 76 名绑定角色的原生更新后状态；m000 连续 710 轮又固定 59.930491 Hz 观察门/主候选扫描节拍和角色顺序 | 剩余运行期全局随机消费者的逐次对齐、稳定 MOD/Remake 真人完整输入通关差分和 Windows 11/更多硬件跨机器验证 |

表中“原版两段启动影片顺序”仅指取证目录和媒体状态机仍可验证该事实；现代
发行流程已经停播并从 Copy 试玩包剔除影片文件，不再构成产品启动步骤。
...
另有六个“加载一关、提交一个动作或边界、立即退出”的进程私有短探针，固定了
随机选人、手枪攻击、地图地雷拾取、B 掩埋和背包丢弃的 10 条选中/接令调用，
并以 S 视线命令作为零调用反例；m000 存读档边界另把完整世界重建的原生实现性
随机抖动与现代精确状态恢复分开。它们同样不是通关测试，尚待补证的是证据窗口外
任意长输入轨迹。
机器可读覆盖表同时逐项约束 32 个调用者和 119 个直接调用点：IDA 入站引用、
PE 入口与十二关进程轨迹共同排除了 3 个休眠的重载/地图创作调用点；正式关卡
实际可达的 107 个调用点中，106 个已有实时消费者，1 个由精确载入检查点恢复，
调用点缺失数为 0。动态创建/删除入口已经逐项分类；尚未闭合的是输入后的无界
时序，不能用
“调用点目录已闭合”冒充任意长输入轨迹已经等价。

type 8/10 已纠正为原生的双 actor 生命周期：actor 84/85 先消费四次局内工厂
取数，触发后另建 actor 62，原部署物最后再消费四次派生/基类析构取数；不再把
部署物本身当作 actor 62 粒子容器。该链路及物理存档已通过十二关无界面世界
闭环。手动掉落物、actor 90 观察标记和 actor 78 藏尸替换也已接入相同的工厂/
析构顺序；这些检查不需要人工尝试通关。

`m010-burial-completion-v1` 现把 B 命令从按键、尸体点击、未提前完成一直追到
严格第 101 次结算：稳定 MOD 与 Remake 均让原尸体退出活动世界、只创建一个
actor 78，并保留原版完成后不会立即清零的 `GoalKind=4`，五检查点严格比较
零差异。该短探针只操作隔离游戏进程，不移动系统鼠标，也不尝试通关。

“资源和任务结构已恢复”不等于“游戏已经复刻完成”。目前已经是带真实碰撞、寻路、动态角色、实际战斗、右下角实时地图、五列背包、特殊对象、完整局内存读档和通用任务事件的十二关可玩原型。`real_mission_world_loop_test.gd` 会逐关加载真实 MOD 资产，通过产品世界入口执行营救、拾取、击杀、任务掉落、炸药消耗、爆破、四区占领和撤离；当前 809 项检查、155 次世界动作、30 个实体存档恢复点和 140 个目标视口事件通过。十二关初始状态和每个稳定 MOD 必需目标完成点都会写入真实磁盘，再逐份重建整张关卡并校验规范化状态哈希。`real_mission_failure_matrix_test.gd` 另从全新关卡状态执行 43 条隔离失败轨迹，逐名覆盖 25 个稳定规则必需队员身份、8 个护送 scene、三关精确限时边界及四个修复 profile，并用非必要角色死亡和 m008 稳定规则手动引爆作反例；它不靠直接注入失败事件。`real_input_world_test.gd` 验证第一关的 F2、鼠标左键、M 和 E；`real_input_campaign_journey_test.gd` 再以 427 项检查和 648 个仅限目标视口的事件逐关覆盖角色定位、移动、R/C、W/A、M、F1、Esc、S、普通攻击、B 掩埋、物理快速存读、主要失败、键盘重玩与 `original` 编辑运行时隔离。十二关原版 `FLIPMISSION` 还通过 720 项成对检查证明输入到胜利状态的转换一致，但明确不计作非作弊通关。上述输入和失败门禁都不移动、锁定或裁剪系统鼠标。`campaign_performance_probe.gd` 进一步在真实 1920×1080 窗口中双轮运行十二关；第二轮最终门禁的 33,130 帧整体 P95 18.193 ms、P99 20.236 ms、零个 >50 ms。上述门禁仍是确定性自动操作，不等于稳定 MOD/Remake 两边的非作弊真人完整通关录像。十二关导演和难度已有可执行第一版，其中补写对白、镜头时长、教程文本、AI 策略和数值明确标为 `remake_editorial`，不能冒充原版内容。严格 `original` 配置不会创建这套导演或 AI 协调器，也不播放编辑型任务 cue；原简报与 m011 已恢复结局仍可播放。只有主动选择 `easy/normal/hard` 才启用补写层。
...
非均匀表才拆列，均匀表保持单个 draw item。静态场景、移动角色、177 个门
状态（97 个关闭、80 个已开放）、拾取物、爆炸物和特殊世界对象均使用同一实现。
为避免首次切换姿态时扫描整张地图造成停顿，载入阶段会收集当前关卡所有
运行时可达动作/方向的多格足印，统一预计算一次与门状态同步的紧凑 L3
`PackedByteArray` 通行位图；构造每名角色时不再重复全图扫描，首次转向也
不再补算。最终 12 关 1920×1080 双轮长时回归的 33,130 个采样帧为
P95 18.193 ms、P99 20.236 ms、零个超过 50 ms 的长帧，且全程不控制
系统鼠标。
...
- 鼠标进入战场水平 32 像素、垂直 48 像素渐变区会按同一曲线平滑加速卷屏；垂直方向另有 100ms 缓释，避免窗口边框和底栏上沿造成单帧断流。下边缘以底部工具栏上沿为准，因此窗口和全屏模式都可平滑垂直卷屏。左右/下方向键、中键拖动和滚轮缩放仍可辅助操作（原版 `↑` 是强制目标按住键）；光标使用原 GFL 16 `mouse.spr`，且不会捕获、吸附或移动系统鼠标；
- `Alt+Enter` 可随时在桌面全屏与已保存的窗口尺寸之间切换，并持久化本次选择；
- `PageUp` / `PageDown` 仅在调试构建中切换上一关或下一关。

普通启动不再播放 logo 和历史 CG，而是直接打开十二关选择页；便携复制包也不会包含这两段 OGV，从而减少体积和首屏等待。启动页与 `Esc` 菜单均提供十二关自由选择，并只读标记当前关、真实完成关和顺序战役前沿；选择关卡本身不会伪造完成记录。菜单另提供 10 个手动存档槽、`quick`、滚动检查点和胜利 `autosave`；任务失败时画面灰化，标题为“任务失败”，正文模板为 `任务失败：%s\n可重新开始本关，或从多槽存档选择器读取进度。`（`%s` 为失败原因），继续/保存按钮不可用。掩埋后的敌方 scene、type 78 藏尸处及双容器、未完成 B 命令的执行者/目标/计数、type 8/10/11、导演节拍与 AI 协作状态均随局内存档恢复；一次性 type 90 观察标记不持久化。胜利演出未看完的自动档会恢复胜利对白/镜头及结局，确认完成后再自动保存一次。菜单可持久化显示模式、VSync、帧率上限、窗口尺寸、UI/文字缩放、经典/现代操作、经典/现代规则、剧情/普通/困难/自定义难度、按键映射、分通道音量、字幕与辅助功能；任何模式都不会捕获、吸附或移动系统光标。存档携带完整运行配置和内容身份，读档前会恢复相同语义，避免数值和导演状态漂移。音乐/环境声由独立播放器进入 `Music`，影片音轨也进入 `Music`，不会误走语音或 SFX。
...
- 167 个带移动脚印的交互门在规划阶段即作为 A* 通道；关门仍保持原图和 L2 视线遮挡，角色靠近时自动开门，玩家也可直接点击。另识别 59 个没有开关动画、画面本来就是敞开的门洞/院门，仅释放与当前门体相交的 L2/L3 源格；第一关 25 个院门和门洞均进入真实路径回归，且不会误开远处复用同一 scene 编号的墙体。
- 敌人移动动画按 SPR 步幅、保持 tick 和当前速度换算帧间隔，不再用固定慢帧率造成“滑行”；60Hz 物理状态在渲染帧之间插值，镜头则保持逐渲染帧更新，避免高刷新率显示器上的单位跳步。产品模式会把相邻巡逻兵组织成一名路线领队和最多三名跟随者；跟随者使用旋转队形槽、独立 A*、追赶限速，并随领队停步和转向。
- 底栏整条 62px 区域均拦截世界输入；五名角色、当前武器、观察、地图和系统按钮都有悬停提示和有效点击动作。当前武器区紧邻右侧动作区，点击与 `Tab` 相同，会切换下一件武器。右下角小地图可单击或按住拖动定位镜头，并会清除旧的卷屏速度，绝不把同一次点击穿透为角色移动命令。
- 活物选择和战斗状态只通过头顶血条表达，不再绘制红色地面圆圈。缺少原图的弹药与物品改用弹药箱/物资包占位图；逐行分层精灵也不再叠加缺图人形，消除了容易误解的“圆圈减号”和“i”形图块。
...
19045 / GTX 1050 Ti 第二轮参考基线共 33,130 帧，整体 P95 18.193 ms、P99
20.236 ms、最大 45.129 ms，零个 >50 ms 帧；最密集的 m004 为 P95
19.086 ms、P99 23.791 ms，66 名敌军实际移动。第二遍十二关结束时静态
内存仅比第一遍增长 4,351,916 字节（4.15 MiB），12 次热载入均低于
3.234 秒，最慢冷载入为 4.162 秒。精简结果见
...
- [现代化第二轮持续改进实施方案](docs/现代化第二轮持续改进实施方案.md)
- [现代化第二轮实施日志](docs/现代化第二轮实施日志.md)
- [现代化第二轮验收报告](docs/现代化第二轮验收报告.md)
- [原生内容包与 MapEditor 工作流](docs/原生内容包与MapEditor工作流.md)
