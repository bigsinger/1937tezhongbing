...
全局随机恢复还包含十二关 5,845 个完整 60 Hz 更新轮、29,662 条覆盖 20 个
调用点的主/次搜索、阻塞重试、跟踪反应、共享计数、路线等待与追击条件事件，
以及 107 组五点搜索。十二关在该窗口内均已通过调用总数、顺序、返回值和最终
LCG state 校验。m000 另有一条明确选择的 413 轮、24,586 次调用的短移动输入
分支：前 297 轮与无输入窗口逐次相同，第 298/358 轮的两次移动确认均在玩家
actor 18 的更新槽消费，最终状态和往返位置也逐项一致。该分支由无界面探针从
离线记录回放，不启动可见游戏、不控制系统鼠标，也不尝试通关；尚未建立目录的
其他玩家输入仍会退出证据通道，不能据此宣称任意输入轨迹已经完全一致。
...
需要在后台完成全部逻辑/真实资产检查、但当前桌面不能弹出测试窗口时，可直接
运行 `powershell -ExecutionPolicy Bypass -File .\tools\Verify.ps1
-GodotExecutable D:\Godot\Godot_v4.7.1-stable_win64_console.exe
-SkipWindowedChecks`。该开关只跳过窗口性能和产品截图探针，不跳过十二关无界面
资源、任务、输入状态、存档或随机流检查，也不会控制系统鼠标。

仅复现 GitHub Actions 的无原始资源检查时可再加 `-SkipRealAssetChecks`；该开关
会跳过本地 `LocalAssets` 门禁，因此不能单独作为真实十二关交付依据，必须与
相应的逐关真实资源测试结果配套记录。
...
原生界面另有独立的双分辨率门禁。稳定 MOD 的 1024×768 主表面先固定
F1、物品/武器栏、小地图、暂停和失败菜单；`Capture-ModernOverlayParity.ps1`
再离屏比较同一 Remake 构建的 1024×768 与 1920×1080 原生裁剪。物品栏、
武器栏、F1 和失败菜单逐像素相同，小地图因现代视口镜头框扩大而保留受控差异，
近似像素为 99.3402%、边缘相关为 0.9583。该工具不发送输入、不推进关卡，
也不提交本地采集图。
