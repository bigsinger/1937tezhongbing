...
另有四个“加载一关、提交一个动作、立即退出”的进程私有短探针，固定了随机
选人、手枪攻击、地图地雷拾取、B 掩埋的 9 条选中/接令调用，并以 S 视线命令
作为零调用反例；它们同样不是通关测试。尚待补证的是背包放置、存读档命令后的
原版时序，以及任意长输入轨迹。
...
调用点缺失数为 0。尚未闭合的是输入后的无界时序和动态删除生命周期，不能用
“调用点目录已闭合”冒充任意长输入轨迹已经等价。

type 8/10 已纠正为原生的双 actor 生命周期：actor 84/85 先消费五次工厂
