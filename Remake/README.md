...
规则集与难度分离、schema 2 存档、辅助功能、中英文产品文本、AI 反馈、十二关
...
| 玩法 | 原版反向寻路、十二关玩家障碍路线的实际轨迹形状差分（4 像素硬门限、最差 3.420 像素）、空间索引动态占位、确定性窄通道会车、稀疏图实时动态规划与密集图巡逻预热/后缀/静态失败复用、多格足印不可达精确预检、门增量导航、实际战斗；四渲染队列；右侧五列背包；6/7/9 投射物；type 8/10 的 actor 62 精确伤害/几何/警报/五轮效果粒子，actor 61/62 主体、type 63—65 永久焦痕及 type 72—76 抛物线弹跳碎片，与 type 11 注意力保持；S 的方向/L2 遮挡空心扇形、type 90/GFL 341 唯一观察标记与 B 的 type 78/GFL 64 藏尸/双容器/101 tick 闭环；实时右下角地图；启动页/菜单内十二关原生自由选择；十槽原子存档、schema 0/1→2 迁移、运行时/内容/工作区身份与未来版本只读保护、原 SAV/SI 单向导入及真实关卡恢复、十二关胜利进度磁盘闭环、按键重映射、分通道音量、失败重玩；原 WAV/简报、原版唯一两段启动影片顺序、确定性事件回放和十二关双轮十分钟窗口性能门禁；十二关原版启动随机流检查点、772 名角色构造值、656 名观察门角色顺序、初始化后 769 条首轮完整调用及 76 名绑定角色的原生更新后状态；m000 连续 710 轮又固定 59.930491 Hz 观察门/主候选扫描节拍和角色顺序 | 剩余运行期全局随机消费者的逐次对齐、稳定 MOD/Remake 真人完整输入通关差分和 Windows 11/更多硬件跨机器验证 |
...
不再补算。最终 12 关 1920×1080 双轮长时回归的 33,105 个采样帧为
P95 17.604 ms、P99 19.363 ms、零个超过 50 ms 的长帧，且全程不控制
系统鼠标。
...
19045 / GTX 1050 Ti 参考基线共 33,105 帧，整体 P95 17.604 ms、P99
19.363 ms、最大 45.438 ms，零个 >50 ms 帧；最密集的 m004 为 P95
18.172 ms、P99 20.196 ms，67 名敌军实际移动。第二遍十二关结束时静态
内存仅比第一遍增长 2,667,836 字节，12 次热载入均低于 3.0 秒，最慢冷
载入为 3.961 秒。精简结果见
