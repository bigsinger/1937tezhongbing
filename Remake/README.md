...
1，证明并非只扣库存。十条轨迹逐项比较两个有序容器、mode 和当前攻击类型；
巡逻相位和在途投射物的目标生命截帧只作诊断，不会被误当成物品差异。
