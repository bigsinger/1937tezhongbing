...
| 精灵动画 | 980 个精灵、2,775 个动画组、11,898 帧全部转换；20 动作 × 9 方向语义；精确帧保持；攻击末帧命中与死亡末帧保持；GFL 270/272 换装与 GFL 470/900 特殊对象帧接线 | 逐层 baseline、其余动作过渡和特殊锚点校准 |
...
障碍返回路线还严格恢复原版等成本阶梯的纵向优先顺序，使最后一段以东北
方向进入目标并保留正确停步朝向，而不依赖 `AStarGrid2D` 的任意平局结果。
