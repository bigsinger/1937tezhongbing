...
本仓库的 `Mod/` 通过 Git LFS 发布稳定游戏内容；`Remake/` 仍采用“源码、
格式说明和任务数据入库，批量转换资源由本地导入器重建”的组织方式，主要
目的有两个：
...
发布策略与这里的 Remake 转换产物策略是两个不同层次。

## 本地导入模型

默认使用 `tools/Import-ModAssets.cmd` 读取仓库稳定 `Mod/`，导入器只读
检查 `repository-mod-12-level-20260729` 哈希 profile，然后将规范化结果
写入 `Remake/LocalAssets/`：
