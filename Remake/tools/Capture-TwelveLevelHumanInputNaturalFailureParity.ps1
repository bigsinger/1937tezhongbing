...
$runtimePrefix = 'mod-human-input-natural-failure-runtime-'

# Every ground cell was observed to accept the original click and naturally
# lead the required player into enemy fire.  m002 instead reproduces the
# original pistol-noise lure because its starting area is navigation-isolated.
$routes = @(
    [pscustomobject]@{ level = 1;  scene = 1436; x = 43; y = 33;  lure = -1 },
    [pscustomobject]@{ level = 2;  scene = 1994; x = 87; y = 240; lure = -1 },
    [pscustomobject]@{ level = 3;  scene = 886;  x = 0;  y = 0;   lure = 840 },
    [pscustomobject]@{ level = 4;  scene = 1150; x = 34; y = 185; lure = -1 },
    [pscustomobject]@{ level = 5;  scene = 2629; x = 60; y = 19;  lure = -1 },
    [pscustomobject]@{ level = 6;  scene = 663;  x = 13; y = 178; lure = -1 },
    [pscustomobject]@{ level = 7;  scene = 1458; x = 14; y = 18;  lure = -1 },
    [pscustomobject]@{ level = 8;  scene = 2325; x = 36; y = 34;  lure = -1 },
    [pscustomobject]@{ level = 9;  scene = 753;  x = 33; y = 26;  lure = -1 },
    [pscustomobject]@{ level = 10; scene = 1709; x = 18; y = 76;  lure = -1 },
    [pscustomobject]@{ level = 11; scene = 1590; x = 6;  y = 23;  lure = -1 },
    [pscustomobject]@{ level = 12; scene = 1176; x = 60; y = 21;  lure = -1 }
)

if ([string]::IsNullOrWhiteSpace($OutputDirectory)) {
    $OutputDirectory = Join-Path $remakeRoot (
        'LocalAssets\qa\twelve-level-human-input-natural-failure-' +
...
if (-not ('NaturalMissionFailureIniV1' -as [type])) {
    Add-Type -TypeDefinition @'
using System.Runtime.InteropServices;
public static class NaturalMissionFailureIniV1
{
...
function Set-IsolatedIniValue {
    ...
    if (-not [NaturalMissionFailureIniV1]::WritePrivateProfileString(
...
try {
    foreach ($level in $Levels) {
        $route = @($routes | Where-Object level -eq $level)[0]
        if ($null -eq $route) {
            throw "Natural failure route is missing for level $level."
        }
        ...
        Write-Host "Observing stable MOD human-input failure for $levelId..."
        $successfulModOutput = ''
        foreach ($attempt in 1..2) {
            ...
            $probeArguments = @(
                ...
                '--native-natural-failure-ms=45000',
                '--contact-observation-ms=1800')
            if ([int]$route.lure -gt 0) {
                $probeArguments += "--native-lure-scene=$($route.lure)"
            }
            & $modProbe @probeArguments
        ...
        if ([string]::IsNullOrWhiteSpace($successfulModOutput)) {
            throw "Stable MOD natural-failure probe failed for $levelId."
        }
        $modTrace = Join-Path $successfulModOutput "mod-$scenarioId.json"
        if (-not (Test-Path -LiteralPath $modTrace -PathType Leaf)) {
            throw "Stable MOD natural-failure trace is missing: $modTrace"
        }
        ...
        Write-Host "Replaying Remake human-input failure for $levelId..."
        & $GodotExecutable @(
            '--headless',
            '--path',
            $gameRoot,
        ...
        $results.Add([pscustomobject][ordered]@{
            ...
            mod_input_kind = [string]$comparison.evidence.reference_input_kind
            remake_input_kind = [string]$comparison.evidence.candidate_input_kind
        ...
        if (Test-Path -LiteralPath $resolvedRuntime) {
            for ($attempt = 0; $attempt -lt 8; ++$attempt) {
                try {
                    ...
                    if ($attempt -eq 7) {
                        Write-Warning (
                            'The isolated natural-failure runtime remains ' +
...
$markdown.Add('# Twelve-level human-input natural mission failure parity')
$markdown.Add('')
$markdown.Add(
    ...
    'evaluator must select `required_character_lost`. No actor HP, mission ' +
    'result, system cursor or global focus is written by the probes.')
$markdown.Add('')
$markdown.Add(
...
foreach ($result in $results) {
    $markdown.Add(
        '| {0} | {1} | {2} | {3} | {4} | {5} | {6} |' -f @(
            ...
            $result.mod_input_kind,
            $result.remake_input_kind,
            $result.check_count,
...
Write-Host "Human-input natural failure parity summary: $summaryMarkdown"
if (-not $allPassed -and -not $AllowMismatch) {
    throw 'One or more human-input natural failure comparisons failed.'
}
