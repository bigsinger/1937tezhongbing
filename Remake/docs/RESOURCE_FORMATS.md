...
当前已接入的玩法字段还包括：ext5/6/8 的计时动作上限、计数和进度显示；ext10 以 `scene_index` 保存的追随目标；ext11 丢到地面的物品数量；ext15 已接受的地面坐标移动命令；ext24 五步局部搜索计数；ext31 L2/L3 动态占位开关；ext32/33 重合双航点守卫的固定朝向及恢复开关；ext40 m002/m004 一次性护送招募完成标志。它们分别由 `sub_4553B0`、`sub_455760`、`sub_45D2A0`、`sub_4583F0`、`sub_458A80`、`sub_45E4B0`、`sub_451B70`、`sub_469820`、`sub_45E950`、`sub_4590E0` 和 `sub_459290` 的读写路径证明。ext19/`+0x21C` 与 ext30/`+0x274` 在支持 EXE 中没有正式 actor 消费者，在十二关及现有七份 SAV 中也始终为 0，因此定性为“当前支持版本休眠零槽”，继续按偏移命名而不猜测。其后的 24 个值会被原加载器读入临时区后丢弃；解析器仍完整保留它们以支持审计和无损往返，十二关 19,199 个实体中的 460,776 个尾值均为 0。`game/data/original_actor_dormant_fields.json` 固定逐关源哈希、实体数和非零计数，`Test-OriginalActorDormantFields.ps1` 会在 MOD 资源实体化时调用同一解析器重建目录并逐字比较；这使“零值/无消费者”成为可回归的数据契约，而不是只写在文档里的判断。
...
2. ext19/`+0x21C` 与 ext30/`+0x274` 已在当前支持 EXE/十二关范围内定性为休眠零槽；若未来支持其他发行版，再研究其潜在消费者、L4 在其他工具/版本中的用法，以及 L5 的编辑器写入流程，这些不再是当前十二关复刻阻塞项；
