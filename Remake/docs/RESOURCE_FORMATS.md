...
ResourceTool 现会批量生成 `m000`—`m011` 的 `terrain.png`、`level.json` 和 `navigation.bin`，并写出 `levels/index.json`。Godot 可按启动参数或 `PageUp` / `PageDown` 加载十二关，并已具备基于 L3 的 A* 寻路及基于 L2 的格线视线核心。完整交互、敌人决策循环和战斗事件仍在开发中。
...
当前已恢复目标依赖、计数、限时、失败和胜利骨架。`m011` 的原程序初始化会反复覆盖爆破目标指针，导致实际只检查最后一个 scene 1353；复刻任务数据按简报把 scene 1348—1353 绑定为六个不同目标，并要求按 `scene_index` 去重计数六次。通用状态机尚不直接校验绑定白名单，后续关卡事件发送器必须只从当前关卡锚点发出事件。详细证据、逐关图和自动/人工边界见 [任务恢复说明](MISSION_RECOVERY.md)。
...
解析器测试由人工生成的微型二进制 fixture 覆盖正常和错误边界，不把批量原版字节提交到仓库。本地已知版本的批量审计还验证了 34/34 IBLOCK、45/45 TLG、980/980 SPR、2,775/2,775 动画组、11,898/11,898 帧、十二个 VWF/SLIST、19,199 个实体、1023 条 DBL 记录，以及 GFL/SLF 的完整引用关系。

仍需研究的重点：

1. DBL 精灵元素数组与实体足印、交互区域之间的关系；
2. SLIST 扩展数组、L4 在其他工具/版本中的用法，以及 L5 的编辑器写入流程；
3. 攻击命中帧、动作过渡和部分 triplet/lookup 参数的玩法语义；
4. 把射击、爆破、救援、物品和出口等世界事件接入任务状态机；
5. 敌人警报传播、听觉、尸体发现、难度、剧情对白、镜头演出和存档格式。
