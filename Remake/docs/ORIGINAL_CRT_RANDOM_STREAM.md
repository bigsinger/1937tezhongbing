...
会应用到对应 runtime actor；其余首轮调用当前先准确消费并推进全局流，
但路线等待、追击和搜索的首轮字段副作用仍要逐项按反编译证据接线。这样既
消除了首轮之后的流错位，又不把尚未恢复的副作用冒充为完成。原版独有但
Remake 不构造的环境对象不会被伪造出来。

## 当前运行时接线

已接入同一可存档/回放流的部分包括：

- 十二关初始化后的首个完整更新轮次及 actor 绑定观察门结果；
- actor 62 爆炸效果粒子；
