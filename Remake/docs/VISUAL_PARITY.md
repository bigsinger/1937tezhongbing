...
顶底 HUD 现已进入独立门禁。`PsdCompositeImage` 将 207 张原版界面 PSD
转换为本地 PNG；运行时使用 GFL 1137—1140 组成 62px 底栏，并直接采用
五名角色的选中/未选中/死亡头像以及观察、地图、系统按钮。静态分析
`sub_441660/sub_43FB70` 又固定了五张 50×20 “角色弹药”图（GFL
1125/1153/1186/1197/1214）、角色折叠顺序、首格 `(53,1)` 和红色数量文字
的相对坐标 `(17,2)`。版本化基线
`game/data/original_hud_layout_baseline.json` 固定 1024×768 和
1920×1080 的顶部状态、底栏、首头像和三个 50×50 按钮矩形；
`original_hud_runtime_test.gd` 用合成资源执行 212 项 HUD/覆盖层无原始数据断言，另由
...
直接显示 PSD 1254。
`original_overlay_asset_baseline.json` 固定 108 张 PNG 的 SHA-256、十二张地图
尺寸、两种桌面分辨率锚点，以及 `mouse.spr` 九个热点、16 帧和 2 tick
帧保持。真实资源门禁逐文件重算哈希，产品探针在 1024×768、1920×1080
各执行 55 项布局/可见性断言并只保存压缩目标视口图。新增的 25 张资源包括
PSD 1094 小按钮底框和正式十二关各自的普通/高亮标题；运行时严格映射
`m000`—`m011`，并排除“雷区伏击战”“内讧”“最后任务”三组未使用标签。

上述三层现在也有稳定 MOD 最终合成证据。`Capture-VisualParity.ps1` 支持
`-Overlay help|items|weapons|minimap|pause|failure`：原版侧只向隔离进程的 DirectInput
回放队列发送一次对应快捷键，再从 cnc-ddraw 主表面读取结果；Remake 侧只
截取自己的目标视口。物品/武器、小地图和暂停菜单按原生矩形裁剪，避免把产品顶部状态
文字及不同阶段的底栏混入面板结论；失败界面则通过进程内伤害入口立即触发，
不移动角色、不自动打关。PSD 1129 还按原 DirectDraw 规则恢复了
...
| 任务失败 | 282×94 | 14.819 | 82.9184% | 0.5753 | 0.4980% |
| 小地图 | 336×166 | 2.159 | 98.0081% | 0.9418 | 0.0072% |

同一批原生覆盖层又完成了 1024×768 → 1920×1080 的第二跳校准。透明/色键
界面在 GameShell 下方放置确定性纯黑测试层，以排除不同地图区域造成的假差异；
F1 自带原版全黑底；小地图保留真实世界和现代视口应当更大的镜头框。测试只读取
Godot 自己的目标视口纹理，不发送任何输入，也不尝试推进或完成关卡：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass `
  -File .\tools\Capture-ModernOverlayParity.ps1 `
  -GodotExecutable D:\Godot\Godot_v4.7.1-stable_win64_console.exe
```

| 现代视口样本 | 原生范围 | MAE | 近似像素 | 边缘相关 | 新增黑洞 |
|---|---:|---:|---:|---:|---:|
| 物品栏（隔离层） | 276×421 | 0.000 | 100.0000% | 1.0000 | 0.0000% |
| 武器栏（隔离层） | 276×421 | 0.000 | 100.0000% | 1.0000 | 0.0000% |
| F1 原生内容 | 640×480 | 0.000 | 100.0000% | 1.0000 | 0.0000% |
| 任务失败（隔离层） | 282×94 | 0.000 | 100.0000% | 1.0000 | 0.0000% |
| 小地图（含镜头框） | 336×166 | 0.820 | 99.3402% | 0.9583 | 0.0018% |

精简指标、两端裁剪哈希、原生锚点和输入隔离声明已并入
`original_overlay_visual_parity_baselines.json`；本地采集图仍不提交。

顶底 HUD 的五组原生像素样本另存为
`game/data/original_hud_visual_parity_baselines.json`，由
`Test-OriginalHudVisualParityBaseline.ps1` 校验；同样只提交度量与哈希，不提交
原版截图：

| HUD 样本 | 比较范围 | MAE | 近似像素 | 边缘相关 | 新增黑洞 |
|---|---:|---:|---:|---:|---:|
| m000 顶部空闲角色 | 50×20 | 3.312 | 94.4000% | 0.9962 | 0.0000% |
| m001 顶部三角色/弹药 30 | 150×20 | 4.297 | 95.9333% | 0.9962 | 0.0000% |
| 1024 完整底栏 | 1024×62 | 2.188 | 98.3540% | 0.9606 | 0.0851% |
| 1920 左侧原生控件岛 | 512×62 | 2.034 | 98.3115% | 0.9561 | 0.0851% |
| 1920 右侧三按钮 | 150×50 | 3.093 | 98.7067% | 0.9929 | 0.0000% |

背包边缘门限单独设为 0.85。任务失败面板的 PSD 1093/1095/1260/1261、
172×50/132×38 原生尺寸、中心偏移和等权灰度由哈希与布局门禁严格固定；
仅“任务失败”“回主界面”两段动态中文使用独立 RGB565 字形门限，因为原版
DirectDraw/GDI 光栅器无法在 Godot 中逐边缘复用。颜色、近似像素和缺失像素
仍有实测上界。这些专用门限都不放宽世界渲染门限。
原版“开始游戏”会直接进入第一关，稳定 MOD 的自由选关页是外部增强，因此
不存在可诚实声称为原版的自由选择器整屏像素样本。该页改用原始资源哈希、
十二关精确映射、3×4 布局和双分辨率目标视口门禁验收，不虚构像素差结论。
顶底 HUD 和 1920×1080 原生模态层的像素合成差已经闭合；尚未完成的是不同
DPI/多显示器环境的跨机器像素验收。
