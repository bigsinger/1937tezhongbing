...
`original_hud_runtime_test.gd` 用合成资源执行 92 项 HUD/覆盖层无原始数据断言，另由
`product_ui_probe.gd` 对本地真实资源生成压缩窗口图和布局 JSON。探针只读
目标视口，不操作系统鼠标。

背包、小地图和 F1 已进入第二份独立静态门禁：背包直接使用 PSD 1129 的
`276×421` 底图，命中格固定为 `(13 + 50×列, 40 + 84×行)`，武器与物品
使用 50×50 普通/选中 PSD 双态；十二张 IBLOCK 地图保持原生尺寸并贴在
62px 底栏上方；F1 直接呈现居中的 IBLOCK 1047 `640×480` 原图。
`original_overlay_asset_baseline.json` 固定 62 张 PNG 的 SHA-256、十二张地图
尺寸、两种桌面分辨率锚点，以及 `mouse.spr` 九个热点、16 帧和 2 tick
帧保持。真实资源门禁逐文件重算哈希，产品探针在 1024×768、1920×1080
各执行 42 项布局/可见性断言并只保存压缩目标视口图。

上述三层现在也有稳定 MOD 最终合成证据。`Capture-VisualParity.ps1` 支持
`-Overlay help|items|weapons|minimap`：原版侧只向隔离进程的 DirectInput
回放队列发送一次对应快捷键，再从 cnc-ddraw 主表面读取结果；Remake 侧只
截取自己的目标视口。物品/武器和小地图按原生矩形裁剪，避免把产品顶部状态
...
尚未完成的是暂停、失败、选择器、其余顶底 HUD 以及 1920×1080 模态层的
逐像素合成差；这些继续保留为显式差距。
