...
`original_hud_runtime_test.gd` 用合成资源执行 154 项 HUD/覆盖层无原始数据断言，另由
...
直接显示 PSD 1254。
`original_overlay_asset_baseline.json` 固定 83 张 PNG 的 SHA-256、十二张地图
...
尚未完成的是选择器、其余顶底 HUD 以及 1920×1080 其余模态层的
逐像素合成差；这些继续保留为显式差距。
