...
- 相对资源路径解析后必须仍位于转换目录内；
- PNG 使用 `Image`/`ImageTexture` 加载，纹理和动画组按路径缓存；同一角色的
  多个动作在一次关卡重建内共享只读 `sprite.json` 解析文档，重建后释放文档
  缓存而保留会话级动作纹理，兼顾冷加载与长会话内存；
...
IBLOCK 1047。暂停菜单还按原主表面恢复 PSD 1095 石纹底框、八组普通/悬停
文字、`(207,266)+40×行` 几何和等权灰度背景；“制作人员”直接显示 PSD 1254。
任务失败层按原主表面恢复 PSD 1093 标题框、PSD 1095/1260/1261 双按钮、
横排几何与等权灰度背景；现代恢复入口保留为“回主界面”的二级页。现代难度、
显示、音量和按键设置也保留为二级页。自由选关使用 PSD 1094 与正式十二关
各自的普通/高亮原版标题，保持增强入口但不伪装成原版内置界面。108 张界面 PNG、十二张地图、
九个光标热点和 16 个光标帧由
`original_overlay_asset_baseline.json` 固定哈希与尺寸；另一个不含原图的
`original_overlay_visual_parity_baselines.json` 固定稳定 MOD 与 Remake 的
F1 整屏、物品/武器面板、暂停菜单、任务失败及小地图裁剪像素指标。采集只使用目标进程自己的
DirectInput 回放队列和 DirectDraw 主表面，不操作桌面输入；同一文件还固定
1024×768 → 1920×1080 的第二跳原生合成指标。物品、武器、F1 和失败菜单
在确定性隔离背景下逐像素相等，小地图保留现代视口更大的镜头框并通过专用
边缘门限。该第二跳完全不发送输入，也不运行关卡流程。
...
`GameShell` 管理原版八项暂停菜单、原版制作人员页、现代设置二级页、十槽选择器、按键重映射、四通道音量、窗口/桌面全屏/无边框最大化、减少自动镜头运动、2 倍原版光标、失败灰化和五列背包。它还在独立 `GameHudRoot` 中组合五张原版 50×20 顶部角色/弹药图、GFL 1137—1140 的 62px 底栏、五名角色三态头像/生命条和三个原图按钮；`Main` 只提交角色状态并接收观察、地图、系统及头像选择信号，不让 HUD 反向持有世界角色对象。项目关闭固定逻辑画布拉伸，以窗口真实像素驱动相机和锚点布局，所以现代桌面能多渲染地图而不是放大 1280×720。显示模式和 HUD 都不捕获、裁剪或移动系统光标。世界左键提交选择/移动/攻击/使用，左 `Ctrl` 或 `↑` 按住时进入强制目标；世界右键只拖框，菜单右键松开返回。右下角地图属于独立 HUD，不暂停 SceneTree；它优先显示原版逐关静态目标图，并增加实时敌我/任务点、镜头框和点击卷屏。`MediaDirector` 在切关时显示原简报图；音乐/环境声和影片音轨进入 `Music`，模态对白与可并发角色语音进入 `Voice`，攻击、命中、非语音警报和 UI WAV 进入 `Sfx`，媒体 `Esc` 在松开时关闭/跳过。角色语音只在现有槽都忙时按需扩展，不抢占正在播放的声音；当前关卡队员的全部移动应答 WAV 会在世界构造阶段静默解码进持久缓存，不启动播放、不推进变体种子，因此第一次地面命令不会把冷音频解码、分配和 A* 同时压到一个显示帧。导演节拍/教程/持久事件和 AI 姿态/增援预算、已掩埋敌人的 scene 索引及特殊对象等可变状态均随 `GameSessionState` 保存恢复。通用中立角色的新增行为、真人体验驱动的难度再校准、真人完整通关差分和更多 Windows 物理机样本属于发布后的研究与社区验证边界，不是本轮现代化自动验收的未实现功能。

严格 `original` 的关内演出闭包已经完成：原程序没有脚本对白、任务镜头或逐关
教程，因而该配置不构造增强导演；F1 全局帮助、简报和结局继续走恢复媒体路径。
启动影片只保留取证目录与状态机测试，现代产品入口已停播。剩余架构工作不再包含
“逐关原版演出恢复”或“1920 模态层校准”。

## 6. 现代化运行服务边界

现代化采用渐进式抽离，不推倒已经通过十二关差分验证的模拟核心。`main.gd`
仍是组合根和旧调用面的兼容门面，但新增职责通过以下对象获得独立测试边界：

```text
Main（组合根）
├── RuntimeServiceHub
│   ├── GameCommandBus             类型化命令与最近命令轨迹
│   ├── RuntimePerformanceMonitor  帧/物理/导航/载入/首次操作指标
│   ├── NavigationRequestQueue     单帧预算和延后队列
│   ├── WorldSpatialIndex          动态对象分桶和 scene 常数时间索引
│   ├── PerceptionScheduler        敌军感知稳定错峰
│   ├── CheckpointManager          关卡开始/目标滚动检查点
│   ├── RuntimeDiagnostics         隐私安全诊断包
│   └── ApplicationFocusPolicy     失焦暂停和恢复状态机
├── LevelLoadPipeline              后台文件读取/解码、主线程节点创建
├── RuntimeSettingsApplier         显示、帧率、音频和辅助设置
├── ModernDifficultyPolicy         规则集之外的独立 AI/难度参数
├── LocalizationService            中英文产品文本目录
├── CombatFeedback                 射程、命中率、声音和失败原因
├── HistoryArchive                 十二关历史背景与史实边界
└── MissionStatistics              任务过程与结算统计
```

空间索引是宽相位优化，不取代 L2/L3 的权威遮挡和导航；候选对象仍由原规则或
现代规则精确过滤。感知调度只改变不同敌军落在哪个扫描帧，不改变同一角色的
确定性查询次序。后台载入只处理无 Godot 对象的字节与 JSON，场景树、纹理和
信号始终在主线程创建。

`DynamicOccupancyGrid` 的多格移动净空缓存以 `PackedByteArray` 保存，不再为
密集地图常驻数十万个 `Vector2i`。缓存键同时包含关卡命名空间、导航尺寸和初始
L3 位图哈希；关卡重建可复用只读基线，而门等运行时变化写入独立副本。若稀疏
地图确实需要坐标数组，则按需从位图物化，因此优化不会放宽路径约束。

60 Hz 移动循环使用 `Vector4` 值类型传递“位置、已用时间、到点标志”，对外
测试接口才转换为字典；朝向未变化时不重复解析动画组。人形角色和军犬采用
运行时单格移动占位，车辆及大型对象继续保留原资源的多格足迹。自动开门也不再
执行“全部门 × 全部移动角色”的笛卡尔扫描，而是以最大门图像接近边界作为
