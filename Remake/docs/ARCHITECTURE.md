...
- 攻击、投掷、近战、受伤/死亡等动作虽然已经转换并可由通用加载器读取，但尚未由战斗事件触发。

因此“全部序列帧已处理”描述的是数据管线能力，不等于完整角色状态机已经实现。
...
`scene_bindings` 当前是任务数据中的预期绑定，`MissionState` 本身只做条件匹配和去重，不负责验证 scene 白名单。后续事件发送器必须从当前关卡已加载的实体与 `task_anchors` 产生事件，并为关键关卡增加绑定测试。

主场景切关时会加载相应任务图并显示目标列表。当前世界还没有把“角色获救、敌人被击毙、物品取得、爆破、出口到达”等实际玩法事件送入 `MissionState`，所以任务图是可执行的数据骨架，还不是可以从头打通的任务流程。
...
当前可控队员使用关卡中对应角色的坐标和已转换动画。玩家与 faction 1 敌人都注册进同一个 `DynamicOccupancyGrid`：源 L2/L3 只读，以源 `ReferenceX/Y` 的八连通分量恢复足印，运行时单独维护角色足印、目标预留、密集移动段检查和第三方视线遮挡。L3 A* 禁止斜穿贴角障碍；敌人读取原巡逻点、方向、感知类型和默认武器，执行巡逻、发现、追击、攻击距离判定、失去目标后的最后位置搜索。退化巡逻点和拥挤重规划使用确定性错峰退避，避免 A* 重算风暴。中立角色移动、可破坏物、伤害/弹药、受伤死亡和任务事件仍未全部接通。
