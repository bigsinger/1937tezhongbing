...
可以自动恢复的是媒体身份、两段启动影片顺序、原版不存在关卡间影片这一闭合结论、逐关简报/示意图、结局图、声音语义类别、角色声线和可重复播放接口。无法从当前文件自动证明的是逐场剧情对白原文、说话时机、镜头运动和角色站位；这些信息必须通过原版实机录像核对后写入对白与镜头数据，不能仅凭文件体积或名称猜测，也不会通过自动通关机器人臆造。
