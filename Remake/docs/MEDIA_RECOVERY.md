...
- `legacy_animation_audio_test.gd` 与 `legacy_explosion_visual_test.gd` 不读取原媒体，验证 action/帧边界、人物信号、爆炸主体、逐粒子请求身份和连续请求语义；`director_main_wiring_test.gd` 验证精确索引路由及通用音效去重。
...
主体和 GFL 23/25/379/380 燃烧粒子都走同一精确索引通路；存在帧组原音效时，
旧的通用攻击/死亡事件音会被抑制，避免同一动作播放两份声音。连续声音按
