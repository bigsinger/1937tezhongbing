...
该表描述补写导演素材，不会覆盖任务判定。m006、m008、m009、m011 在默认 `stable_mod` 下会停用与实际原控制流冲突的整套编辑导演；选择 `repaired` 后才启用表中的接头、手动引爆、全清敌或六目标节奏。这样不会用现代补写内容反向改变稳定 MOD 兼容规则。
...
`original` 继续只使用已经恢复的坐标广播、同 tick 命令仲裁和五点搜索，
不创建 `MissionAiCoordinator`。`easy/normal/hard` 的增强协同则在警报产生时
立即冻结目标坐标、来源坐标和当时速度；亚秒级分享延迟结束后，不再读取目标的
新位置，也不把活目标引用保留给接收者。因此敌人只能调查“最后看见/听见的位置”，
不会因为玩家已经离开而获得透视追踪。

协同小组按距离和 scene index 稳定选出后，分别取得 `lead`、`left_flank`、
`right_flank` 和可选 `rear_block` 槽位。每个槽位都给出“宽侧翼 → 紧凑侧翼 →
原始证据点”的候选序列；`EnemyUnit` 使用同一套 `DynamicOccupancyGrid` A* 逐项
验证。若路径只是被墙截断的远端 partial route，终点与槽位相差超过 48 像素时
会拒绝该候选并尝试下一个，所以窄门、围墙和单通道会自然收缩队形，而不是让敌人
卡在墙后。候选查询使用不清除、不预留目标格的只读预览入口，只有最终选中的路径
才提交占位目标，不会扰动原巡逻路线。槽位角色、命令序号和最终可达坐标进入局内存档；读档后重新请求该坐标
的路径。尚未送达、含节点引用的亚秒级警报仍按既有策略视为瞬态，不写入 JSON。

这套编队是明确标记的 `remake_editorial` 智能增强，不冒充原版行为；无窗口回归
验证冻结坐标、角色分配、不可达回退、无活目标引用和状态往返，不需要启动或通关
任何关卡。
