...
随机流：飞行 actor 成功局内工厂四次，碰撞/终点先析构四次，再创建 actor 60 或
请求 actor 61；actor 60 动画结束后再析构四次。schema 3 快照保存飞行/命中
actor 的归属，物理读档只恢复归属和动画进度，不重复消费存档前的工厂取数。

原 EXE 还保留 effect 3 / delivery mode 2 / actor 58 的残影实现，但它不是隐藏
武器或遗漏关卡能力。效果分发器只有两个调用者，其实际 effect 常量并集不含 3；
六类正式投射 profile 也都不使用 mode 2。完整的 19 个工厂点、20 个删除点和
不可达证明保存在 `../SDK/dynamic-actor-lifecycle-sites.json`，因此 Remake
不会为十二关凭空增加该分支。
...
原版模式下前者显示当前项目的直接数量，后者恒为零。schema 4 存档继续保存数量
模式、拥有状态和当前武器；schema 1 的“弹匣 + 备弹”旧档会在迁移时合并成一个
直接数量。敌方角色也保留原版 `+0x22C` 有序容器；`infinite_ammo` 对敌人只表示
...
| `B` 后左击符合原版过滤条件的阵亡目标 | 下达原版 `GoalKind 4`，自动寻路到 32×16 邻格范围并在严格大于 100 tick 后完成；目标移除、单次结果对象、完成后状态保留及存读档均有差分/自动化门禁 |
...
项目 43/45、actor 84/85/62、GFL 470/900、type 8 的 faction 1 与 `32 × 16` 触发、type 10 的 100 tick，以及上述伤害/几何/警报/粒子寿命均来自恢复路径。actor 84/85 通过四次取数的成功局内动态工厂建立；触发后另建 actor 62，最后才消费原部署物四次派生/基类析构取数，不能把两者合并为同一粒子节点。`0x5BBBC` 仅属于 SAV actor 状态恢复，不再错误并入局内创建。汽油桶的 actor 53、生命哨兵 8、效果类型 5 和 actor 62 链路也已由 35 条 VWF 状态与 `sub_4551B0`/`sub_4656C0`/`sub_4554A0` 闭合。早期秒制通用 `LandMine` 重制状态机及其默认数据已经删除；项目 43 只走原版 type 8 / actor 84 路径。
...
`sub_4554A0` 已确认爆炸伤害不查询地形视线遮挡，Remake 已按该规则恢复。动态 actor 的全部
19 个创建点和 20 个管理器删除点已经分类；健康正式局内路径均已接线，仍显式
保留的缺口仅是切关检查点清理与原生资源/内存失败回滚。m006 actor 101 的建立、type 22 接收、析构和
