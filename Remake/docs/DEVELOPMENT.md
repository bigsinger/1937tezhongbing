...
当前验证计数基线为：171 项 .NET 合成格式/媒体目录测试、123 项 Godot 核心逻辑、160 项战斗/任务、82 项投射物/背包、120 项世界交互、46 项无资产媒体、21 项产品壳、81 项存档/设置和 93 项确定性回放。存在完整 `LocalAssets` 时再追加 6,753 项真实关卡/任务绑定与 310 项真实媒体审计；计数变化必须由功能或 fixture 变化解释，不能只改文档或放宽断言。
...
type 8/10/11 的专用运行时仍未实现，但字段级取证已经收窄其边界：type 8 创建 actor 84 / GFL 470、消费物品 43，并由存活 faction 1 进入 32×16 范围触发；type 10 创建 actor 85 / GFL 900、消费物品 45，在第 100 个 world tick 爆炸；type 11 不直接结算伤害，当前路径表现为 AI 抑制/受控且不消费物品 99，其完整建立、持续和解除生命周期仍待恢复。实现这些类型时必须新增可测试的专用状态对象，不得把证据简化成通用即时命中。详见 [导航、动态占位、敌人感知与攻击](NAVIGATION_AND_COMBAT.md) 与 [投射物、背包与世界交互物](PROJECTILES_AND_INVENTORY.md)。
...
`game_shell.gd` 是暂停态产品壳，统一管理 `Esc` 菜单、`M` 战术地图、`B`/`I` 背包和任务失败灰化层。所有覆盖层都用 `PROCESS_MODE_WHEN_PAUSED`，打开时暂停 SceneTree；失败层不能“继续”，只能重玩、读取或退出。`tactical_map_view.gd` 只消费主场景提供的地形、敌我/任务标记和镜头矩形，不自行推断任务规则；m010 的四个占点必须来自现有任务 scene 绑定。

`game_settings.gd` 管理版本化 `user://settings.json`。当前菜单公开全屏、主音量、字幕、任务简报和鼠标边缘卷屏；全屏策略固定为当前桌面分辨率。底层 schema 已预留 windowed/fullscreen/borderless、分通道音量和提示类别，但没有菜单入口的字段不能写成玩家已经可配置。设置变更后应立即应用并原子保存。

`game_save_store.gd` 管理 `user://saves/<slot>.json`，`game_session_state.gd` 负责主场景可变状态的捕获/恢复。`F5` 与菜单保存写 `quick`，`F9` 与菜单读取选择时间最新的有效槽；胜利写 `autosave`。任务失败时禁止覆盖有效存档。存档边界包括：

- 关卡、任务耗时、完成/进度/去重、失败和持久事实；
- 队员/敌人/护送角色的位置、阵营、生命/死亡、选择、背包/武器/弹药，以及 AI 巡逻/搜索和护送关系；
- 已激活任务 scene、公共物品、剩余拾取物、可爆物状态、任务掉落、地雷和未结算投射物；
- 镜头和战役完成/解锁进度。
