...
同一组稳定双快照也恢复了独立的 `+0x22C` 武器容器：十二关共 660 个
精确角色、761 个有序武器条目和 67 个空容器，其中 27 名玩家占 83 个
条目。当前装备类型使用运行时进入/稳定快照的一致值，不再回退到 VWF
作者字段；敌军同样持有原版容器，但 `sub_456DF0` 证明普通敌军攻击不扣除
其中数量。Remake 的 `infinite_ammo` 因而只表达“不消耗”，不再表示
“没有武器容器”。
...
`sub_450200`、`sub_459200`、`sub_459290`、`sub_459370`、`sub_45EA70`、
`sub_45EC20` 与 `sub_45EE00` 已把 faction 1 掩护从单一外观值升级为完整
运行时生命周期：

- 换装计数严格使用 `counter > 100`，即第 101 个 60 Hz 角色 tick 生效；
...
- 暴露后连续 101 个无观察 tick 恢复 faction 1；观察会清零恢复计数，
  伪装状态下掩埋尸体被看见也会立即暴露；
- 第 8 关铁蛋 scene 2298/runtime type 9 初始同样是 faction 1，但只在完成
  目标/容器取物且被 640 参数方向边界和 L2 视线实际目击时暴露；观察者收到
  物品坐标而不是活目标，101 个无观察 60 Hz 更新后恢复 faction 1。普通攻击
  和仅仅接近物品不触发这条 `sub_45EC20` 路径；
...
- 点击有直接效果的物品立即使用；点击其他物品会关闭物品栏并进入一次性
  放置状态，下一次左键点击地面会给该角色下达原版状态 9 命令；
- 角色先按 A* 前往点击位置，途中物品仍留在背包；只有抵达最终可行走点后，
  才会强制消费一份并创建 runtime type 等于物品 ID 的状态 3 世界对象。
  右键可在尚未下达地面命令时取消选择；原程序不存在用于立即丢弃的 `T`
  快捷键；
...
- 若精确文件袋掉落同时属于任务 `role_drop`，运行时把任务字段合并到该
  原版掉落，不再额外复制第二份文件袋；
- 角色背包、伪装状态、尚未拾取的地面物品、尚在寻路的状态 9 放置命令、
  敌人的物品目标、随机状态及木偶/毒酒/骨头/香烟计数器全部随局内存档
  恢复；途中存档不会提前扣除或复制物品。

放置命令的证据链来自 `sub_43E050`、`sub_450200`、`sub_457F00`、
`sub_458A80`、`sub_458360` 与 `sub_4583F0`：物品格点击先写全局选择，
六类诱饵物品把命令类型设为 9；地面点击再把物品 ID 和目标坐标写入角色，
寻路抵达后才生成世界 actor 并通过 `sub_45ADE0(item, 1)` 强制转移一份。

稳定 MOD 与 Remake 的六组差分轨迹进一步关闭了该链路：

- m007 鸡 33：type 5 敌军拾取并保留一份；
- m010 肉罐头 48：type 6 敌军拾取并保留一份；
- m010 降头木偶 49：拾取后强制消费并进入 601 tick 临时控制；
- m010 毒酒 52：拾取后强制消费，第 81 tick 造成 16 点伤害；
- m009 狗骨头 82：type 56 军犬拾取、保留并分神；
- m010 香烟 83：敌军拾取、保留并分神。

六组轨迹都严格核对拾取前后有序容器、mode、世界对象、控制/分神/中毒
状态和边界 tick，结果均为零差异。毒酒目标在死亡动画期间，原版延后到
死亡掉落阶段才清空容器，因此 `after_effect` 只跳过容器时点比较，仍严格
核对 `poison_counter=81`、阈值 80、生命和存活状态；拾取前后容器比较
保持严格。
...
powershell -NoProfile -ExecutionPolicy Bypass -File `
  .\Remake\tools\Capture-InventoryParity.ps1
...
资源，并核对 660 个角色/761 条武器记录、660 个角色/539 条物品记录、
A/W 分栏和存档往返；`Capture-InventoryParity.ps1` 则重放十条攻击/部署
轨迹和上述六条世界物品轨迹，并生成严格 JSON/Markdown 比较。
