...
现有短时证据已离线整理为 5,845 个完整更新轮、29,662 条覆盖 20 个调用点的
条件 actor 事件和 107 组五点搜索；十二关在该窗口内均已逐调用校验。第一关另有
一条 413 轮、24,586 次调用的短移动输入分支，两个命令确认均按原版发生在玩家
角色自己的更新槽。验证只回放离线记录并运行无界面短场景，不启动可见关卡、
不控制鼠标，也不尝试通关；其他尚未取证的输入分支不会被冒充为已恢复。
当前资源转换已升级为 SPR schema 4：1,137 个带声音帧组的 52 个 SLF/GFL
绑定均由离线解析确定，人物动作、爆炸主体和独立燃烧粒子按原版帧边界播放；
验证依靠合成单测、十二关真实资源审计和短时无头加载，不尝试自动打通关卡。
