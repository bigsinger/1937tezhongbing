...
- `OriginalEnemyAI.hpp` 固定原版枪声的严格 640 参数方向边界
  ...
  16 像素世界边界；同时固定正式可达的 type
  16/20/25/27/28/29 直接次级搜索，以及 type 18/19/24/26 的任务包装器
  回落规则、严格 `<128` 插入顺序候选和
...
- `Media.hpp` 由 `media-routes.json` 生成，固定原程序唯一两次影片调用的
  logo→历史片顺序、原始参数和 RVA，并统一描述十二关简报、全局 F1 帮助及
  selector 13 的 640/800/1024 结局图；27 条演示字符串、两个摄像机 setter
  调用和全部相机写入者的闭包证明没有关卡间影片，也没有关内脚本对白、任务
  镜头或逐关教程序列；
...
- `Mission7Exchange.hpp` 固定 m006 的 scene 1457/type 15 携带者、scene
  1460/type 22 接收者、scene 1462/type 100 检测对象、actor 101/GFL 246、
  32/256 距离边界与 `x-16` 放置偏移；两个专用更新入口由地址目录统一生成；
...
反应状态；field 8 写 `+0x1D0` 的计时动作进度显示开关，绝不是阵营。阵营始终
取 DBL。其余正式消费者现也已闭合到计时动作、追随目标 scene index、地面物品
数量、坐标移动命令、五步局部搜索、动态导航占位、固定守卫朝向和一次性护送招募；
只保留 ext19/`+0x21C` 与 ext30/`+0x274` 两个全零槽按偏移命名；它们已由
`Remake/game/data/original_actor_dormant_fields.json` 在十二关 19,199 个实体中
固定为零，并由支持 EXE 的无正式消费者结论定性为当前版本休眠槽。随后的 24 个
序列化值只进入加载器临时区，SDK 将其作为保留尾部建模，避免格式工具静默丢字节；
同一目录会检查全部 460,776 个尾值为零，同时仍要求解析器原样保存非零合成值。
...
`+0x294/+0x298/+0x29C` type 9/type 91 共用的掩护恢复状态/上限/计数。
...
军服 54/青衫 92/项目 99 的容器变化、严格 101 次 60 Hz actor update 切换与
恢复、手枪/匕首目击暴露，以及 type 4/12 和第 2/6/8 关特殊单位的识破规则；
同时固定第 8 关铁蛋 type 9 只有取物被目击才暴露、与 type 91 共用 101 次
无观察恢复的规则。换装、状态转移、type 9/type 91 更新和两条目击扫描函数
RVA 均由同一地址目录生成。
